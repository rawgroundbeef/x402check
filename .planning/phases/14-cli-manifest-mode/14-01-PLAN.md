...
<task type="auto">
  ...
  <action>
    1. Install cli-table3 as a devDependency: `pnpm add -D cli-table3` in packages/x402check/.
...
<task type="auto">
  ...
  <files>packages/x402check/src/cli/fetch.ts</files>
  <action>
...
<task type="auto">
  ...
  <files>packages/x402check/src/cli/detect.ts</files>
  <action>
...
<verification>
1. `pnpm --filter x402check exec tsc --noEmit` passes with no type errors

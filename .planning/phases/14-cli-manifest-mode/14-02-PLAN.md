...
must_haves:
  truths:
    - "x402check manifest.json auto-detects manifest and shows per-endpoint summary table + details"
    - "x402check config.json auto-detects single config and shows existing output (no regression)"
  ...
  artifacts:
    - path: "packages/x402check/src/cli/format.ts"
      ...
      min_lines: 80
    - path: "packages/x402check/src/cli.ts"
      provides: "Main CLI entry point with manifest routing"
      min_lines: 80
  key_links:
    - from: "packages/x402check/src/cli.ts"
      to: "packages/x402check/src/cli/detect.ts"
...
@packages/x402check/src/check.ts
@packages/x402check/tsdown.config.ts
</context>
...
<task type="auto">
  ...
  <files>packages/x402check/src/cli/format.ts</files>
  <action>
  ...
  <verify>
    Run `pnpm --filter x402check exec tsc --noEmit` to verify types compile.
...
<task type="auto">
  ...
  <action>
    **Rewrite `src/cli.ts`** to compose the CLI modules from Plan 01 and Task 1.
    ...
    2. **Help text:**
       ...
       - `x402check <manifest.json>` example
       - `x402check -` stdin example
    ...
    **tsdown.config.ts update:**
    ...
    One concern: cli-table3 has a dependency (string-width). tsdown should bundle both transitively. Verify by running `pnpm --filter x402check build` after changes.
  ...
  <verify>
    1. `pnpm --filter x402check build` succeeds (CLI binary builds)
    2. `node packages/x402check/dist/cli.mjs --help` shows updated help with manifest examples and --header flag
    3. `node packages/x402check/dist/cli.mjs --version` prints version
    4. Create a test manifest file and run: `node packages/x402check/dist/cli.mjs /path/to/manifest.json` — should show summary table + endpoint details
    5. `node packages/x402check/dist/cli.mjs --json /path/to/manifest.json` outputs parseable JSON
    6. `node packages/x402check/dist/cli.mjs /path/to/valid-v2-base.json` still works (no regression on single configs)
...
<verification>
1. `pnpm --filter x402check build` succeeds with no errors
2. `pnpm --filter x402check exec tsc --noEmit` passes

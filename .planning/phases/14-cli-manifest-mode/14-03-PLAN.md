...
must_haves:
  ...
  artifacts:
    - path: "packages/x402check/test/fixtures/valid-manifest.json"
      provides: "Test fixture for valid manifest with 2+ endpoints"
    - path: "packages/x402check/test/fixtures/invalid-manifest.json"
      provides: "Test fixture for manifest with at least one failing endpoint"
    - path: "packages/x402check/test/cli.test.ts"
      provides: "Updated CLI tests covering manifest mode"
      min_lines: 150
  key_links:
    - from: "packages/x402check/test/cli.test.ts"
      to: "packages/x402check/dist/cli.mjs"
...
<task type="auto">
  ...
  <verify>
    Files exist and contain valid JSON: `node -e "require('./packages/x402check/test/fixtures/valid-manifest.json')"` and similar for invalid.
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>packages/x402check/test/cli.test.ts</files>
  <action>
    ...
    **`describe('cli -- dash stdin for manifest')`:**
    ...
    Important: All tests use the same `run()` helper already in cli.test.ts. Add the FIXTURES constant for the new manifest fixture paths. Build must be run before tests (`pnpm --filter x402check build`).
  </action>
  <verify>
    1. `pnpm --filter x402check build` succeeds
    2. `pnpm --filter x402check test` — all tests pass (both existing and new)
...
<verification>
1. `pnpm --filter x402check build && pnpm --filter x402check test` — all pass

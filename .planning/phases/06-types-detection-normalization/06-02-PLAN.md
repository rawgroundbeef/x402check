...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/x402check/src/detection/detect.ts"
      ...
      contains: "detect"
    - path: "packages/x402check/src/detection/normalize.ts"
      provides: "normalize() function, normalizeV2, normalizeV1ToV2, normalizeFlatToV2"
      contains: "normalize"
    - path: "packages/x402check/src/detection/index.ts"
  ...
  key_links:
    ...
    - from: "packages/x402check/src/index.ts"
      to: "packages/x402check/src/detection/index.ts"
      via: "re-export"
...
```
  ...
  <verify>
Run `cd /Users/rawgroundbeef/Projects/x402check && pnpm --filter x402check run typecheck` -- must pass.
Run `cd /Users/rawgroundbeef/Projects/x402check && pnpm --filter x402check run test` -- all tests pass including detection.test.ts (expect 25+ test cases).
  </verify>
  <done>
...
2. `pnpm --filter x402check run test` passes -- all 25+ test cases in detection.test.ts

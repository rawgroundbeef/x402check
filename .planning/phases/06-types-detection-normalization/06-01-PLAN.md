...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/x402check/src/types/validation.ts"
    ...
    - path: "packages/x402check/src/types/errors.ts"
      ...
      contains: "as const"
    - path: "packages/x402check/src/types/parse-input.ts"
      provides: "parseInput utility for string | object handling"
      contains: "JSON.parse"
    - path: "packages/x402check/src/types/index.ts"
    ...
    - path: "packages/x402check/src/index.ts"
      ...
      contains: "export"
  key_links:
    - from: "packages/x402check/src/types/index.ts"
    ...
    - from: "packages/x402check/src/index.ts"
      to: "packages/x402check/src/types/index.ts"
      via: "re-export"
      pattern: "export.*from.*types"
---

<objective>
Create the complete type system, error vocabulary, and input parsing utility for the x402check SDK.
...
All exports must be named exports (no default). Use `export const` and `export interface`/`export type`.
  ...
  <verify>
Run `cd /Users/rawgroundbeef/Projects/x402check && pnpm --filter x402check run typecheck` -- must pass with zero errors.
...
Then write a test file at `packages/x402check/test/types.test.ts`:
...
```
  ...
  <verify>
Run `cd /Users/rawgroundbeef/Projects/x402check && pnpm --filter x402check run typecheck` -- must pass.
Run `cd /Users/rawgroundbeef/Projects/x402check && pnpm --filter x402check run test` -- all tests pass including new types.test.ts.
  </verify>
  <done>
...
<verification>
1. `pnpm --filter x402check run typecheck` passes with zero errors
2. `pnpm --filter x402check run test` passes -- both existing smoke test and new types.test.ts
